...
```js
 var tellMe = ["...human...why you taking pictures of me?...", "...the catnip made me do it...","...why does the red dot always get away..."]; 
 for(var i =1; i<= 20; i++)  
 { 
...
```js
var favoriteMovies = ["Troy","Saw","22jumpStreat","the300","BigStan"];
```
...
```js
var ninja = ["Donatello", "Leonardo", "Raphael", "Michaelangelo"];for(var i =0; i<ninja.length;i++)
{ 
...
{  if(toggle)  {  console.log(ninja[i].toUpperCase()); 
}
else
{  console.log(ninja[i].toLowerCase()); 
} 
toggle = !toggle;
}
```
